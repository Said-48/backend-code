...
def list_activities(current_user):
    try:
        # Use query without deprecated methods
        activities_query = ActivityLog.query.order_by(ActivityLog.created_at.desc())
        activities_paginated = paginate(activities_query, request)
